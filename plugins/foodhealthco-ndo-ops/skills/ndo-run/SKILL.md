...
- Confirm the `.env` (plugins-repo, NDO checkout, or `~/.config/ndo-run/.env` — see `discover_env_file` for the chain) has `NDO_DEV_DATABASE_URL`, `NDO_PROD_DATABASE_URL`, `NDO_GCS_BUCKET`, `FHS_API_URL`, `FHS_API_TOKEN`. The runner will fail loudly if any are missing. **Object storage is GCS now** (post DO→GCP migration): the runner uploads the op/send CSV to `gs://$NDO_GCS_BUCKET/<key>` (e.g. `ndo-files-prod`) where the management commands read it, authenticating with your gcloud ADC — so `gcloud auth application-default login` must be done and you need read+write on that bucket. (The old `DO_SPACES_*` keys are no longer used for this upload.)
- **Database proxy — auto-managed, don't start it by hand.** NDO moved to GCP Cloud SQL (2026-07-17) and is reached through a local `cloud-sql-proxy`. The runner starts the right proxy for `--target` itself (prod → `hero-db-prod:5443`, dev → `hero-db-dev:5444`), reuses one that's already running, and tears down only what it started — so operators no longer run a proxy in a separate terminal. What each machine needs **once**: `cloud-sql-proxy` installed (`brew install cloud-sql-proxy`), `gcloud auth application-default login`, and `roles/cloudsql.client` on the project. If the runner can't bring the proxy up, it exits with the exact fix (install / auth / grant) rather than a raw connection error.
